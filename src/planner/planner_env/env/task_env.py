...
class TaskEnv:
    ...
    def generate_env(self):
        ...
        tasks_time = self.random_value(tasks_num, 1) * self.max_duration
    ...
    def plot_animation(self, path, n):
        ...
        # combs = list(combinations(self.task_dic.keys(), 2))
        # task_squares = [ax2.add_patch(patches.RegularPolygon((task['location'][0] * 10,
        #                                                      task['location'][1] * 10),
        #                                                     int(task['requirements'].sum()) + 3,
        #                                                     0.3, color='b', zorder=100, alpha=0.5)) for task in self.task_dic.values()]
        # ax2.add_patch(patches.Circle((self.depot['location'][0] * 10, self.depot['location'][1] * 10), 0.2, color='r'))
        # lines_t = [ax2.plot([], [], alpha=0.5, color='orange')[0] for _ in combs]
        # frame = 100
        # # for task in self.task_dic.values():
        # #     if task['finish_time']:
        # #         if np.max(task['finish_time'], axis=0)[1] <= frame * self.dt:
        # #             task_squares[task['ID']].set_color('g')
        # for i, l in enumerate(lines_t):
        #     l.set_data([self.task_dic[combs[i][0]]['location'][0] * 10, self.task_dic[combs[i][1]]['location'][0] * 10],
        #                [self.task_dic[combs[i][0]]['location'][1] * 10, self.task_dic[combs[i][1]]['location'][1] * 10])
        # lines_d = [ax2.plot([t['location'][0] * 10, self.depot['location'][0] * 10], [t['location'][1] * 10, self.depot['location'][1] * 10], alpha=0.5, color='orange', zorder=100)[0] for t in self.task_dic.values()]
        # # plt.show()
        # ############################
        # ####################################
        # from itertools import combinations
        ...
        # combs = list(combinations(self.agent_dic.keys(), 2))
        # lines_t = [ax3.plot([], [], alpha=0.15, color='pink')[0] for _ in combs]
        # frame = 40
        # agent_triangles = [ax3.add_patch(patches.RegularPolygon((a['trajectory'][frame + 1, 0] * 10,
        #                                                         a['trajectory'][frame + 1, 1] * 10), 3, 0.2, color='r'))
        #                    for a in self.agent_dic.values()]
        # for i, l in enumerate(lines_t):
        #     l.set_data([self.agent_dic[combs[i][0]]['trajectory'][frame + 1, 0] * 10, self.agent_dic[combs[i][1]]['trajectory'][frame + 1, 0] * 10],
        #                [self.agent_dic[combs[i][0]]['trajectory'][frame + 1, 1] * 10, self.agent_dic[combs[i][1]]['trajectory'][frame + 1, 1] * 10])
        # # lines_d = [ax2.plot([t['location'][0] * 10, self.depot['location'][0] * 10], [t['location'][1] * 10, self.depot['location'][1] * 10], alpha=0.5, color='y')[0] for t in self.task_dic.values()]
        # ###################################
        # ####################################
        # from itertools import combinations
        ...
        # combs = list(combinations(self.agent_dic.keys(), 2))
        # wid = np.array([0.03, 0.6, 0.06, 0.02, 0.2, 0.08, 0.1, 0.1, 0.1, 0.03, 0.05, 0.05, 0.05, 0.05, 0.05])
        # wid = wid/wid.sum()
        # lines_t = [ax4.plot([], [], alpha=0.5, color='pink', linewidth=np.random.rand() *10)[0] for _ in range(self.tasks_num)]
        # frame = 40
        # agent_triangles = [ax4.add_patch(patches.RegularPolygon((self.agent_dic[5]['trajectory'][frame + 1, 0] * 10,
        #                                                         self.agent_dic[5]['trajectory'][frame + 1, 1] * 10), 3, 0.2, color='r', zorder =100))
        #                     ]
        # task_squares = [ax4.add_patch(patches.RegularPolygon((task['location'][0] * 10,
        #                                                      task['location'][1] * 10),
        #                                                     int(task['requirements'].sum()) + 3,
        #                                                     0.3, color='b',alpha=0.5)) for task in self.task_dic.values()]
        # for i, l in enumerate(lines_t):
        #     l.set_data([self.agent_dic[5]['trajectory'][frame + 1, 0] * 10, self.task_dic[i]['location'][ 0] * 10],
        #                [self.agent_dic[5]['trajectory'][frame + 1, 1] * 10, self.task_dic[i]['location'][1] * 10])
        # # lines_d = [ax2.plot([t['location'][0] * 10, self.depot['location'][0] * 10], [t['location'][1] * 10, self.depot['location'][1] * 10], alpha=0.5, color='y')[0] for t in self.task_dic.values()]
        # plt.show()
        # ###################################
        plt.subplots_adjust(left=0, right=0.85, top=0.87, bottom=0.02)
        lines = [ax.plot([], [])[0] for _ in self.agent_dic.values()]
        ...
        ax.add_patch(patches.Circle((self.depot['location'][0] * 10, self.depot['location'][1] * 10), 0.2, color='r'))
        if plot_robot_icon:
            agent_triangles = []
            for a in self.agent_dic.values():
    ...
    def get_grouped_tasks(self):
        ...
        for keys, values in grouped_tasks.items():
            # agent_v[keys] = (len(values) / self.tasks_num + np.sum(self.get_matrix(values, 'time_mean'))/np.sum(self.get_matrix(self.task_dic, 'time_mean'))) * keys
    ...
    def execute_by_route(self, path='./', method=0, plot_figure=False):
        ...
        while not self.finished and self.current_time < 100:
            if self.reactive_planning:
...
if __name__ == '__main__':
    ...
    env = TaskEnv((5, 5), tasks_range=(10, 10), traits_dim=1, max_coalition_size=2, seed=0)
    while not env.finished:
        ...
        env.agent_update()
        for agent in decision_agents:
            print(env.get_current_task_status(env.agent_dic[agent]))
            print(env.get_current_agent_status(env.agent_dic[agent]))
            if not env.agent_dic[agent]['returned']:
                mask = env.get_unfinished_tasks()
                if np.sum(mask) == 0:
                    env.agent_step(agent, 0)
                else:
                    env.agent_step(agent, env.rng.choice(env.tasks_num, p=mask / np.sum(mask)) + 1)
                env.task_update()
                env.agent_update()
        env.finished = env.check_finished()
    # env.generate_traj()
    # env.plot_animation('./', 0)
    print()
